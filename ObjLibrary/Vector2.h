...
//  project
//
//  Purpose: To project this Vector2 onto another Vector2.
//  Parameter(s):
//    <1> project_onto: The Vector2 to be projected onto 
//  Precondition(s):
//    <1> isFinite()
...
//    <3> !project_onto.isZero()
//  Returns: N/A
//  Side Effect: This Vector2 is set to be its projection onto
//               project_onto.  The projection is the component
//               of this Vector2 with the same (or opposite)
//               direction as project_onto.
//
	void project (const Vector2& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(!project_onto.isZero());

		operator=(getProjection(project_onto));
	}
...
//
//  Purpose: To project this Vector2 onto another Vector2
//           without crashing if the other Vector2 is zero.
//           This function is slower than project.
//  Parameter(s):
//    <1> project_onto: The Vector2 to be projected onto 
//  Precondition(s):
//    <1> isFinite()
...
//               project_onto.  The projection is the component
//               of this Vector2 with the same (or opposite)
//               direction as project_onto.  If project_onto is
//               zero, this Vector2 is sset to the zero vector.
//
	void projectSafe (const Vector2& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());

		operator=(getProjectionSafe(project_onto));
	}

//
//  projectNormal
//
//  Purpose: To project this Vector2 onto another Vector2 that
//           is a normal vector.
//  Parameter(s):
//    <1> project_onto: The Vector2 to be projected onto 
//  Precondition(s):
...
//               direction as project_onto.
//
	void projectNormal (const Vector2& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(project_onto.isNormal());

		operator=(getProjectionNormal(project_onto));
	}

//
//  reject
//
//  Purpose: To subtract off the projection of this Vector2 onto
//           another Vector2.
//  Parameter(s):
//    <1> project_onto: The Vector2 to be projected onto 
//  Precondition(s):
...
//    <3> !project_onto.isZero()
//  Returns: N/A
//  Side Effect: This Vector2 is set to be its old value minus
//               its projection onto project_onto.  The
//               remainder is the component of this Vector2 at
//               right angles to project_onto.
//
	void reject (const Vector2& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(!project_onto.isZero());

		operator=(getRejection(project_onto));
	}
...
//               its projection onto project_onto.  The
//               remainder is the component of this Vector2 at
//               right angles to project_onto.  If project_onto
//               is zero, this Vector2 is unchanged.
//
	void rejectSafe (const Vector2& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());

		operator=(getRejectionSafe(project_onto));
	}
...
//
//  Purpose: To subtract off the projection of this Vector2 onto
//           another Vector2 that is a normal vector.
//  Parameter(s):
//    <1> project_onto: The Vector2 to be projected onto 
//  Precondition(s):
//    <1> isFinite()
...
//               right angles to project_onto.
//
	void rejectNormal (const Vector2& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(project_onto.isNormal());

		operator=(getRejectionNormal(project_onto));
	}
...
//  reflect
//
//  Purpose: To set this Vector2 to the reflection of its old
//           value off of a surface with the specified normal
//           vector.  Despite the name, the surface normal does
//           not have to have a norm of 1.0.
//  Parameter(s):
//    <1> surface_normal: The normal vector of the surface to
...
//    <3> !surface_normal.isZero()
//  Returns: N/A
//  Side Effect: This Vector2 is set to the reflection off a
//               surface with normal vector surface_normal.
//
	void reflect (const Vector2& surface_normal)
	{
		assert(isFinite());
		assert(surface_normal.isFinite());
		assert(!surface_normal.isZero());

		operator=(getReflection(surface_normal));
	}
...
//  Purpose: To set this Vector2 to the reflection of its old
//           value off of a surface with the specified normal
//           vector, without crashing if the surface normal is
//           the zero vector.  Despite the name, the surface
//           normal does not have to have a norm of 1.0.
//  Parameter(s):
//    <1> surface_normal: The normal vector of the surface to
//                        reflect off of
//  Precondition(s):
...
//  Side Effect: This Vector2 is set to the reflection off a
//               surface with normal vector surface_normal.
//               If surface_normal is zero, this Vector2 is
//               unchanged.
//
	void reflectSafe (const Vector2& surface_normal)
	{
		assert(isFinite());
		assert(surface_normal.isFinite());

		operator=(getReflectionSafe(surface_normal));
	}
...
//
//  Purpose: To set this Vector2 to the reflection of its old
//           value off of a surface with the specified
//           normalized normal.
//  Parameter(s):
//    <1> surface_normal: The normal vector of the surface to
//                        reflect off of
...
//               surface with normal vector surface_normal.
//
	void reflectNormal (const Vector2& surface_normal)
	{
		assert(isFinite());
		assert(surface_normal.isFinite());
		assert(surface_normal.isNormal());

		operator=(getReflectionNormal(surface_normal));
	}
